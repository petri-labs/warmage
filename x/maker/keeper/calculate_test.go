...
func (suite *KeeperTestSuite) TestEstimateMintBySwapIn() {
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(1009, 3))
			},
		...
		},
		{
			...
			req: &types.EstimateMintBySwapInRequest{
				MintOut:      sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(2_500000)),
		...
		},
		{
			...
			req: &types.EstimateMintBySwapInRequest{
				MintOut:      sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1_000000)),
				BackingDenom: suite.bcDenom,
			},
			expPass: true,
			expRes: &types.EstimateMintBySwapInResponse{
				...
				MintFee:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(5000)),
			},
		},
		{
		...
		},
		{
			...
			req: &types.EstimateMintBySwapInRequest{
				MintOut:      sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1_500000)),
				BackingDenom: suite.bcDenom,
			},
			expPass: false,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(101, 2))
			if tc.malleate != nil {
				tc.malleate()
			}
...
func (suite *KeeperTestSuite) TestEstimateMintBySwapOut() {
	testCases := []struct {
	...
	}{
		{
			name: "war price too low",
			malleate: func() {
				suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(989, 3))
			},
		...
		},
		{
			...
			expRes: &types.EstimateMintBySwapOutResponse{
				...
				MageIn:    sdk.NewCoin(warmage.AttoMageDenom, sdk.ZeroInt()),
				MintOut:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(985050)), // 1_000000 * 0.99 * (1 - 0.005)
				MintFee:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(4950)),   // 1_000000 * 0.99 * 0.005
			},
		},
		{
			name: "user asked full backing",
		...
		},
		{
			...
			expRes: &types.EstimateMintBySwapOutResponse{
				...
				MintOut:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(995000)), // 10**16 * 10**-10 * (1 - 0.005)
				MintFee:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(5000)),   // 10**16 * 10**-10 * 0.005
			},
		},
		{
			...
			expRes: &types.EstimateMintBySwapOutResponse{
				...
				MintOut:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(615656)),             // 500000 * 0.99 / 0.8 * (1 - 0.005)
				MintFee:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(3094)),               // 500000 * 0.99 / 0.8 * 0.005
			},
		},
		{
		...
		},
		{
			...
			expRes: &types.EstimateMintBySwapOutResponse{
				...
				MageIn:    sdk.NewCoin(warmage.AttoMageDenom, sdk.NewInt(10000_000000_000000)),
				MintOut:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1243750)), // 10**16 * 10**-10 / 0.8 * (1 - 0.005))
				MintFee:   sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(6250)),    // 10**16 * 10**-10 / 0.8 * 0.005
			},
		},
		{
...
func (suite *KeeperTestSuite) TestEstimateBurnBySwapIn() {
	testCases := []struct {
	...
	}{
		{
			...
			malleate: func() {
				suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(991, 3))
			},
		...
		},
		{
			...
			expPass: true,
			expRes: &types.EstimateBurnBySwapInResponse{
				BurnIn:     sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(995976)), // 1_000000 * 0.99 / (1-0.006)
				BackingOut: sdk.NewCoin(suite.bcDenom, sdk.NewInt(1_000000)),
				MageOut:    sdk.NewCoin(warmage.AttoMageDenom, sdk.ZeroInt()),
				BurnFee:    sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(5976)), // 1_000000 * 0.99 / (1-0.006) * 0.006
			},
		},
		{
			name: "full algorithmic",
			...
			expPass: true,
			expRes: &types.EstimateBurnBySwapInResponse{
				BurnIn:     sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1006036)), // 10**16 * 10**-10 / (1-0.006)
				BackingOut: sdk.NewCoin(suite.bcDenom, sdk.ZeroInt()),
				MageOut:    sdk.NewCoin(warmage.AttoMageDenom, sdk.NewInt(10000_000000_000000)),
				BurnFee:    sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(6036)), // 10**16 * 10**-10 / (1-0.006) * 0.006
			},
		},
		{
			name: "zero mage using backing",
			...
			expPass: true,
			expRes: &types.EstimateBurnBySwapInResponse{
				BurnIn:     sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(622485)), // 500000 * 0.99 / 0.8 / (1-0.006)
				...
				BurnFee:    sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(3735)),               // 500000 * 0.99 / 0.8 / (1-0.006) * 0.006
			},
		},
		{
			name: "fractional using max backing",
		...
		},
		{
			...
			expPass: true,
			expRes: &types.EstimateBurnBySwapInResponse{
				BurnIn:     sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1257545)), // 10**16 * 10**-10 / 0.8 / (1-0.006)
				BackingOut: sdk.NewCoin(suite.bcDenom, sdk.NewInt(252525)),          // 10**16 * 10**-10 / 0.8 * 0.2 / 0.99
				MageOut:    sdk.NewCoin(warmage.AttoMageDenom, sdk.NewInt(10000_000000_000000)),
				BurnFee:    sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(7545)), // 10**16 * 10**-10 / 0.8 / (1-0.006) * 0.006
			},
		},
		{
			name: "fractional using max mage",
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			suite.SetupTest() // reset
			suite.setupEstimationTest()
			suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(99, 2))
			if tc.malleate != nil {
				tc.malleate()
			}
...
func (suite *KeeperTestSuite) TestEstimateBurnBySwapOut() {
	testCases := []struct {
	...
	}{
		{
			name: "war price too high",
			malleate: func() {
				suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(1011, 3))
			},
		...
		},
		{
			...
			req: &types.EstimateBurnBySwapOutRequest{
				BurnIn:       sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1)),
				BackingDenom: suite.bcDenom,
			},
			expPass: false,
			expErr:  types.ErrBackingCoinInsufficient,
		...
		},
		{
			name: "full backing",
			req: &types.EstimateBurnBySwapOutRequest{
				BurnIn:       sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(1_000000)),
				BackingDenom: suite.bcDenom,
			},
			expPass: true,
			expRes: &types.EstimateBurnBySwapOutResponse{
				...
				MageOut:    sdk.NewCoin(warmage.AttoMageDenom, sdk.ZeroInt()),
				BurnFee:    sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(6000)), // 1_000000 * 0.006
			},
		},
		{
			name: "full algorithmic",
...
func (suite *KeeperTestSuite) setupEstimationTest() {
	...
	suite.app.OracleKeeper.SetExchangeRate(suite.ctx, warmage.MicroUSWDenom, sdk.NewDecWithPrec(1005, 3))
	...
	suite.app.MakerKeeper.SetPoolBacking(suite.ctx, types.PoolBacking{
		WarMinted:  sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(8_000000)),
	...
	suite.app.MakerKeeper.SetTotalBacking(suite.ctx, types.TotalBacking{
		WarMinted:  sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(8_500000)),
		MageBurned: sdk.NewCoin(warmage.AttoMageDenom, sdk.ZeroInt()),
	})
	...
	suite.app.MakerKeeper.SetAccountCollateral(suite.ctx, suite.accAddress, types.AccountCollateral{
		...
		WarDebt:             sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(6_000000)),
	...
	suite.app.MakerKeeper.SetPoolCollateral(suite.ctx, types.PoolCollateral{
		...
		WarDebt:            sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(8_000000)),
	...
	suite.app.MakerKeeper.SetTotalCollateral(suite.ctx, types.TotalCollateral{
		WarDebt:            sdk.NewCoin(warmage.MicroUSWDenom, sdk.NewInt(10_000000)),
		MageCollateralized: sdk.NewCoin(warmage.AttoMageDenom, sdk.ZeroInt()),
	})
}
