...
> Delegate validators will likely require you to deposit some funds (in War or Mage) which they can use to pay fees, sent in a separate MsgSend. This agreement is made off-chain and not enforced by the Warmage protocol.

The `Operator` field contains the operator address of the validator (prefixed `warvaloper-`). The `Delegate` field is the account address (prefixed `war-`) of the delegate account that will be submitting exchange rate related votes and prevotes on behalf of the `Operator`.
